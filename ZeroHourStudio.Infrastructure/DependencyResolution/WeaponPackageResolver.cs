...
{
    private const int MAX_DEPENDENCIES = 80;
    ...
    public WeaponTransferManifest ResolveWeapons(string unitName, string sourceModPath)
    {
        ...
        var logPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "wpresolver_debug.log");
        File.AppendAllText(logPath, $"\n=== {DateTime.Now:HH:mm:ss} Resolving: {unitName} ===\n");

        // ═══ 1. البحث عن Object block في SageIndex ═══
        var unitDef = _sageIndex.GetMergedDefinition(unitName);
        if (unitDef == null)
        {
            File.AppendAllText(logPath, "Unit not found in SageIndex\n");
            return manifest;
        }

        File.AppendAllText(logPath, $"Found unit in: {unitDef.SourceFile}\n");
        var unitContent = unitDef.RawContent;
        File.AppendAllText(logPath, $"Content length: {unitContent.Length}\n");
        File.AppendAllText(logPath, $"Has WeaponSet: {unitContent.Contains("WeaponSet", StringComparison.OrdinalIgnoreCase)}\n");
        ...
        ExtractUnitBaseDependencies(unitDef, manifest, sourceModPath, logPath);

        // ═══ 3. استخراج أسماء الأسلحة من WeaponSet ═══
        var weaponSlots = ExtractWeaponSlots(unitContent);
        File.AppendAllText(logPath, $"Weapon slots found: {weaponSlots.Count}\n");
        foreach (var (slot, name) in weaponSlots)
        {
            File.AppendAllText(logPath, $"  {slot}: {name}\n");
        }

        if (weaponSlots.Count == 0)
        {
            File.AppendAllText(logPath, $"No weapons found - unit may be unarmed\n");
        }
        ...
        foreach (var (slot, weaponName) in weaponSlots)
        {
            ...
            {
                File.AppendAllText(logPath, $"Skipping NONE weapon in slot {slot}\n");
                continue;
            }

            var package = BuildWeaponPackage(unitName, slot, weaponName, sourceModPath, logPath);
            File.AppendAllText(logPath, $"Package {slot}/{weaponName}: Complete={package.IsComplete}, Deps={package.DependencyCount}\n");
            if (package.IsComplete && package.DependencyCount <= MAX_DEPENDENCIES)
        ...
        if (manifest.TotalDependencies > MAX_DEPENDENCIES)
        {
            File.AppendAllText(logPath, $"MAX_DEPENDENCIES exceeded ({manifest.TotalDependencies}), rejecting all weapons\n");
            manifest.RejectedWeapons.AddRange(manifest.AcceptedWeapons);
            manifest.AcceptedWeapons.Clear();
        }

        File.AppendAllText(logPath, $"Final: {manifest.AcceptedWeapons.Count} accepted, {manifest.RejectedWeapons.Count} rejected, {manifest.UnitDependencies.Count} unit deps\n");
    ...
    /// </summary>
    private void ExtractUnitBaseDependencies(SageDefinition unitDef, WeaponTransferManifest manifest, string sourceModPath, string logPath)
    {
        var content = unitDef.RawContent;
        var depsAdded = 0;
        var unitName = unitDef.Name;

        File.AppendAllText(logPath, $"\n[ExtractUnitDeps] Analyzing unit: {unitName}\n");
        ...
        foreach (var pattern in modelPatterns)
        {
            foreach (Match m in Regex.Matches(content, pattern, RegexOptions.IgnoreCase | RegexOptions.Multiline))
            {
                ...
                if (!string.IsNullOrWhiteSpace(modelName))
                {
                    ...
                    depsAdded++;
                    File.AppendAllText(logPath, $"[ExtractUnitDeps] Model: {modelName}\n");
                }
            }
        }
        ...
        File.AppendAllText(logPath, $"[ExtractUnitDeps] Sounds extracted: {depsAdded} so far\n");
        ...
        foreach (var pattern in locoPatterns)
        {
            ...
            if (locoMatch.Success)
            {
                ...
                depsAdded++;
                File.AppendAllText(logPath, $"[ExtractUnitDeps] Locomotor: {locoName}\n");
                break;
            }
        }
        ...
        foreach (var pattern in armorPatterns)
        {
            ...
            if (armorMatch.Success)
            {
                ...
                depsAdded++;
                File.AppendAllText(logPath, $"[ExtractUnitDeps] Armor: {armorName}\n");
                break;
            }
        }
        ...
        foreach (var pattern in cmdSetPatterns)
        {
            ...
            if (cmdSetMatch.Success)
            {
                ...
                depsAdded++;
                File.AppendAllText(logPath, $"[ExtractUnitDeps] CommandSet: {cmdSetName}\n");
                break;
            }
        }
        ...
        foreach (var kvp in imagePatterns)
        {
            foreach (Match m in Regex.Matches(content, kvp.Key, RegexOptions.IgnoreCase | RegexOptions.Multiline))
            {
                ...
                depsAdded++;
                File.AppendAllText(logPath, $"[ExtractUnitDeps] Image: {imageName}\n");
            }
        }
        ...
        File.AppendAllText(logPath, $"[ExtractUnitDeps] File refs from SageDefinitionIndex: {fileRefCount}\n");
        File.AppendAllText(logPath, $"[ExtractUnitDeps] TOTAL unit dependencies: {depsAdded}\n");
    }
    ...
    private void AddUnitDependency(WeaponTransferManifest manifest, string name, DependencyType type, string sourceModPath, string? iniFile = null)
    {
        ...
        manifest.UnitDependencies.Add(new DependencyNode
        {
            Name = name,
            Type = type,
            FullPath = fullPath ?? $"{{MISSING}}/{name}",
    ...
    private static List<(string Slot, string WeaponName)> ExtractWeaponSlots(string objectContent)
    {
        ...
        File.AppendAllText(logPath, $"[ExtractWeaponSlots] Content length: {objectContent.Length}\n");
        File.AppendAllText(logPath, $"[ExtractWeaponSlots] Has WeaponSet: {objectContent.Contains("WeaponSet", StringComparison.OrdinalIgnoreCase)}\n");

        // Try primary regex first
        var matches1 = WeaponSlotPattern.Matches(objectContent);
        File.AppendAllText(logPath, $"[ExtractWeaponSlots] Regex1 matches: {matches1.Count}\n");
        foreach (Match m in matches1)
        {
            var slot = m.Groups[1].Value.Trim().ToUpperInvariant();
            var name = CleanWeaponName(m.Groups[2].Value.Trim());
            File.AppendAllText(logPath, $"[ExtractWeaponSlots] Regex1 match: {slot} = {name}\n");
        ...
        File.AppendAllText(logPath, $"[ExtractWeaponSlots] Regex2 matches: {matches2.Count}\n");
        foreach (Match m in matches2)
        {
            var slot = m.Groups[1].Value.Trim().ToUpperInvariant();
            var name = CleanWeaponName(m.Groups[2].Value.Trim());
            File.AppendAllText(logPath, $"[ExtractWeaponSlots] Regex2 match: {slot} = {name}\n");
            if (!string.IsNullOrWhiteSpace(name) && seen.Add(slot))
                slots.Add((slot, name));
        }
        ...
        if (slots.Count == 0)
        {
            File.AppendAllText(logPath, $"[ExtractWeaponSlots] Using line-by-line fallback\n");
            var weaponLines = objectContent.Split('\n').Where(l => l.Trim().StartsWith("Weapon", StringComparison.OrdinalIgnoreCase)).ToList();
            File.AppendAllText(logPath, $"[ExtractWeaponSlots] Found {weaponLines.Count} lines starting with Weapon\n");
            foreach (var rawLine in weaponLines)
            {
                var line = rawLine.Trim();
                File.AppendAllText(logPath, $"[ExtractWeaponSlots] Checking: {line.Substring(0, Math.Min(60, line.Length))}...\n");
                ...
                if (parts.Length >= 3)
                {
                    ...
                    if (slotIdx >= 0 && slotIdx + 1 < parts.Length)
                    {
                        ...
                        File.AppendAllText(logPath, $"[ExtractWeaponSlots] Fallback found: {slot} = {name}\n");
                        if (!string.IsNullOrWhiteSpace(name) && seen.Add(slot))
                            slots.Add((slot, name));
                    }
                }
        ...
        File.AppendAllText(logPath, $"[ExtractWeaponSlots] Total slots: {slots.Count}\n");
        return slots;
    }
    ...
    /// </summary>
    private WeaponPackage BuildWeaponPackage(string unitName, string slot, string weaponName, string sourceModPath, string logPath)
    {
        ...
        File.AppendAllText(logPath, $"\n--- Building package for {slot}/{weaponName} ---\n");
        ...
        if (weaponDef == null)
        {
            ...
            File.AppendAllText(logPath, $"Weapon {weaponName} NOT FOUND in SageIndex\n");
            return package;
        }
        ...
        File.AppendAllText(logPath, $"Weapon found: {weaponName} in {weaponDef.SourceFile}\n");

        // استخراج كل تبعيات السلاح (Model, Texture, Audio, FX)
        AddDefinitionDeps(weaponDef, package, sourceModPath, 1);
        File.AppendAllText(logPath, $"After Weapon deps: {package.AllDependencies.Count} total\n");

        // ═══ B. Projectile ═══
        var projectileName = ExtractFirstMatch(weaponDef.RawContent, ProjectilePattern);
        File.AppendAllText(logPath, $"Projectile extracted: {projectileName ?? "(none)"}\n");
        ...
        if (!string.IsNullOrWhiteSpace(projectileName))
        {
            ...
            if (projDef != null)
            {
                File.AppendAllText(logPath, $"Projectile found: {projectileName}\n");
                ...
                {
                    File.AppendAllText(logPath, $"DetonationFX: {detFx}\n");
                    ResolveFxChain(detFx, package, sourceModPath, 3);
                }
            }
            else
            {
                File.AppendAllText(logPath, $"Projectile {projectileName} NOT FOUND\n");
            }
        }
        else
        {
        ...
        {
            File.AppendAllText(logPath, $"FireSound: {fireSound}\n");
        ...
        else
        {
            ...
            if (!string.IsNullOrWhiteSpace(fxName))
            {
                ...
                {
                    File.AppendAllText(logPath, $"Audio from FX: {fxAudio}\n");
                    package.Audio = new WeaponElement
                    {
                        Name = fxAudio,
    ...
    private void AddDefinitionDeps(SageDefinition def, WeaponPackage package, string sourceModPath, int depth)
    {
        ...
        if (package.AllDependencies.Count >= MAX_DEPENDENCIES) return;

        var refs = SageDefinitionIndex.ExtractReferences(def);
        foreach (var sageRef in refs)
        {
            if (package.AllDependencies.Count >= MAX_DEPENDENCIES) break;
