...
public class SageDefinitionIndex
{
            ...
            await Task.Run(() =>
            {
                Parallel.ForEach(iniFiles, file =>
                {
                    try
                    {
                        ...
                        var content = SmartDependencyResolver.DecodeIniBytes(bytes);
                        ParseBlocks(content, file, false);
                    }
                    catch (Exception ex) { BlackBoxRecorder.Record("SAGE_INDEX", "INI_READ_ERROR", $"File={file}, Error={ex.Message}"); }
                });
            });
        }
            ...
            try
            {
                ...
                foreach (var entry in entries)
                {
                    try
                    {
                        ...
                        var content = SmartDependencyResolver.DecodeIniBytes(data);
                        var source = $"{bigFile}::{entry}";
                        bool isHighPriority = Path.GetFileName(bigFile).StartsWith("!!");
                        ParseBlocks(content, source, isHighPriority);
                    }
                    catch (Exception ex) { BlackBoxRecorder.Record("SAGE_INDEX", "BIG_ENTRY_ERROR", $"Entry={entry}, Error={ex.Message}"); }
                }
            }
            catch (Exception ex) { BlackBoxRecorder.Record("SAGE_INDEX", "BIG_ARCHIVE_ERROR", $"Archive={bigFile}, Error={ex.Message}"); }
        }
    }
    ...
    private static SageRefType ClassifyReference(string key, string value)
    {
        ...
        // SpecialPower references
        if (k == "SPECIALPOWER" || k == "SPECIALPOWERTEMPLATE")
            return SageRefType.Upgrade;

        // Science references
        if (k == "SCIENCE" || k == "REQUIREDSCIENCE" || k == "SCIENCEREQUIRED" ||
            k == "GRANTSCIENCE" || k == "KILLERSCIENCE" || k == "PICKUPSCIENCE")
            return SageRefType.Upgrade;

        // ModifierList references
        if (k == "MODIFIERLIST")
            return SageRefType.FXList;
