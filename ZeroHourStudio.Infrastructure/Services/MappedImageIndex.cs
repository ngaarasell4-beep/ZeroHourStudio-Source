    ...
    public async Task BuildIndexAsync(string modPath)
    {
        ...
        foreach (var dir in mappedImageDirs)
        {
            ...
            foreach (var file in Directory.GetFiles(dir, "*.ini", SearchOption.AllDirectories))
            {
                ...
                catch (Exception ex) { BlackBoxRecorder.Record("MAPPED_IMAGE_INDEX", "INI_READ_ERROR", $"File={file}, Error={ex.Message}"); }
            }
        }
        ...
        foreach (var bigPath in bigFiles)
        {
            try
            {
                ...
                foreach (var entry in entries)
                {
                    ...
                    catch (Exception ex) { BlackBoxRecorder.Record("MAPPED_IMAGE_INDEX", "BIG_ENTRY_ERROR", $"Entry={entry}, Error={ex.Message}"); }
                }
            }
            catch (Exception ex) { BlackBoxRecorder.Record("MAPPED_IMAGE_INDEX", "BIG_ARCHIVE_ERROR", $"Archive={bigPath}, Error={ex.Message}"); }
        }
    }
