...
public class CommandSetPatchService
{
    ...
    /// <summary>
    /// حذف زر أمر من CommandSet فصيل معين في slot محدد
    /// </summary>
    public async Task<bool> RemoveCommandButton(string targetModPath, string factionName, int slotNumber)
    {
        try
        {
            var iniRoot = Path.Combine(targetModPath, "Data", "INI");
            ...
            // Find the faction's CommandSet
            var factoryCommandSet = ResolveFactoryCommandSet(factionName);
            if (string.IsNullOrWhiteSpace(factoryCommandSet))
                return false;
            ...
            // Build new block without the specified slot
            var builder = new StringBuilder();
            foreach (var line in commandSetBlock.Lines)
            {
                var trimmed = line.Trim();
                var equalsIndex = trimmed.IndexOf('=');
                if (equalsIndex > 0)
                {
                    var left = trimmed.Substring(0, equalsIndex).Trim();
                    if (int.TryParse(left, out var slot) && slot == slotNumber)
                    {
                        // Skip this slot (effectively removing it)
                        continue;
                    }
                }
                builder.AppendLine(line);
            ...
            // Write the patched version to the AutoGenerated folder
            var patchFolder = Path.Combine(targetModPath, "Data", "INI", "AutoGenerated");
            Directory.CreateDirectory(patchFolder);
            var patchPath = Path.Combine(patchFolder, "CommandSetSlotPatch.ini");
            await File.AppendAllTextAsync(patchPath, builder.ToString(), Encoding.ASCII);

            System.Diagnostics.Debug.WriteLine($"[CommandSetPatchService] Removed slot {slotNumber} from {factoryCommandSet}");
            return true;
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[CommandSetPatchService] RemoveCommandButton ERROR: {ex.Message}");
            return false;
        }
    }
