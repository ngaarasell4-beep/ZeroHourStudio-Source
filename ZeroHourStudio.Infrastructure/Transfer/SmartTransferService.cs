...
public class SmartTransferService : ISmartTransferService
{
    ...
    // ملفات INI مشتركة — يجب دمج البلوكات المطلوبة فقط بدل نسخ الملف كاملاً
    private static readonly HashSet<string> SharedIniFiles = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        IProgress<TransferProgress>? progress = null)
    {
        ...
        try
        {
            ...
            // 3. الحصول على قائمة الملفات للنقل (بدون تكرار)
            var seenTargets = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            var filesToTransfer = new List<DependencyNode>();
            foreach (var node in graph.AllNodes
                .Where(n => n.Status == AssetStatus.Found && n.FullPath != null)
                .OrderBy(n => n.Depth))
            {
                var relPath = GetRelativePath(node.FullPath!, sourcePath);
                var targetKey = Path.Combine(destinationPath, relPath);
                if (seenTargets.Add(targetKey))
                    filesToTransfer.Add(node);
            }
            ...
            if (missingNodes.Count > 0 && _gameArchiveReader != null)
            {
                foreach (var missingNode in missingNodes)
                {
                    ...
                    catch (Exception ex) { BlackBoxRecorder.Record("TRANSFER", "RECOVERY_ERROR", $"File={missingNode.Name}, Error={ex.Message}"); }
                }
            }
            ...
            for (int i = 0; i < filesToTransfer.Count; i++)
            {
                ...
                try
                {
                    ...
                    // === دمج ذكي لملفات INI المشتركة ===
                    var fileName = Path.GetFileName(targetPath);
                    bool isSharedIni = SharedIniFiles.Contains(fileName) && File.Exists(targetPath);
                    ...
                    if (isSharedIni)
                    {
                        ...
                        if (sourceFullPath.Contains("::", StringComparison.Ordinal))
                        {
                            var parts = sourceFullPath.Split(new[] { "::" }, StringSplitOptions.RemoveEmptyEntries);
                            ...
                            sourceContent = System.Text.Encoding.GetEncoding(1252).GetString(data);
                        }
                        else
                        {
                            sourceContent = await File.ReadAllTextAsync(sourceFullPath, System.Text.Encoding.GetEncoding(1252));
                        }

                        // تحليل المصدر والهدف ودمج البلوكات المطلوبة فقط
                        var targetIni = _iniMerger.Parse(targetPath);
                        var sourceIni = _iniMerger.ParseContent(sourceContent);

                        // دمج كل بلوكات المصدر التي تتعلق بالوحدة المنقولة
                        ...
                        if (mergeResult.AddedCount > 0 || mergeResult.ReplacedCount > 0)
                        {
                            ...
                            Debug.WriteLine($"[SmartTransfer] INI Merge '{fileName}': {mergeResult.Summary}");
                        }
                        else
                        {
                            Debug.WriteLine($"[SmartTransfer] INI Merge '{fileName}': لا تغييرات مطلوبة");
                        }
                    }
                    else
                    {
                        // === نسخ عادي للملفات غير المشتركة ===
    ...
    private static void Rollback(List<string> createdFiles, List<(string Original, string Backup)> backups)
    {
        foreach (var created in createdFiles)
        {
            ...
            catch (Exception ex) { BlackBoxRecorder.Record("TRANSFER", "ROLLBACK_DELETE_ERROR", $"File={created}, Error={ex.Message}"); }
        }
        ...
        foreach (var (original, backup) in backups)
        {
            ...
            catch (Exception ex) { BlackBoxRecorder.Record("TRANSFER", "ROLLBACK_RESTORE_ERROR", $"File={original}, Error={ex.Message}"); }
        }
    }
    ...
    private static void CleanupBackups(List<(string Original, string Backup)> backups)
    {
        foreach (var (_, backup) in backups)
        {
            ...
            catch (Exception ex) { BlackBoxRecorder.Record("TRANSFER", "CLEANUP_ERROR", $"File={backup}, Error={ex.Message}"); }
        }
    }
        ...
        IProgress<TransferProgress>? progressReporter)
    {
        ...
        using (var destinationStream = new FileStream(destinationFile, FileMode.Create, FileAccess.Write, FileShare.Read, BufferSize, useAsync: true))
        {
            ...
            catch (Exception ex) { BlackBoxRecorder.Record("TRANSFER", "TIMESTAMP_ERROR", $"File={destinationFile}, Error={ex.Message}"); }
        }
    }
